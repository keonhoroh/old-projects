...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
        ...
        for (double[] arr : wetwet) { //FOR EACH ARRAY WITHIN THE 2D ARRAY
            if (longitude == arr[0] && latitude == arr[1]) {
                for (int i = 2; i < arr.length; i++) {
                        wetwetProbability[i-2] = arr[i];
                }
            }
        }
        
        for (double[] arr : drywet) { 
            if (longitude == arr[0] && latitude == arr[1]) {
                for (int i = 2; i < arr.length; i++) {
                        drywetProbability[i-2] = arr[i];
                }
            }
        }
    }
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        for(int i = 1; i < numberOfDays; i++) {
            double otherDays = StdRandom.uniform();
            if (forecast[i-1] == WET) {
                if ( otherDays < wetwetProbability ){
                    forecast[i] = WET;
                }
                else {
                    forecast[i] = DRY;
                ...
                if (otherDays < drywetProbability ){
                    forecast[i] = WET;
                }
                else {
                    forecast[i] = DRY;
    ...
    public static int bestWeekToTravel(int[] forecast){
        int[] sumDry = new int[forecast.length];
        int dryDays = 0;
        for (int i = 0; i < forecast.length; i++){
            if (forecast.length - (i+1) >= 6){
                for (int k = 0; k < 7; k++){
                    if ( forecast[i+k] ==2 ){
                        dryDays++;
                    }
        ...
        int largestSum = sumDry[0];
        int index = 0;
        for ( int i = 0; i < sumDry.length; i++){
            if (sumDry[i] > largestSum){
                largestSum = sumDry[i];
                index = i;
            }
        }
    ...
    public static void main (String[] args) {
        StdRandom.setSeed(1636066785717L);
