public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
            ...
            if (fragment.length() > 0) {
                ...
                switch (s[0]) {
                    ...
                    case "name":
                        ...
                        break;
                    case "due":
                        dueSigning = Double.parseDouble(s[1]);
                        break;
                    case "length":
                        leaseLength = Integer.parseInt(s[1]);
                        break;
                    case "monthly":
                        monthlyCost = Double.parseDouble(s[1]);
                        break;
                    case "mile/unit":
                        usage = Double.parseDouble(s[1]);
                        break;
                    case "allowance":
                        mileageAllowance = Integer.parseInt(s[1]);
                        break;
                    case "charger":
                        chargerCost = Double.parseDouble(s[1]);
                        break;
                    default:
                        break;
                }
	    ...
	    return new Vehicle(carName, fuelType.equals("gas") ? new Fuel(usage) : new Fuel(usage, chargerCost), new Lease (dueSigning, leaseLength, monthlyCost, mileageAllowance));
	}
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        for (Vehicle v : vehicles) {
            v.setCO2Emission(computeCO2(v.getLease().getLeaseLength(), v.getFuel().getUsage(), v.getLease().getMileageAllowance(),
                (v.getFuel().getType() == Fuel.GAS) ? Fuel.CO2EMITTED_GASCOMBUSTION : Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION));

            v.setFuelCost(computeFuelCost(v.getLease().getLeaseLength(), v.getFuel().getUsage(), v.getLease().getMileageAllowance(),
                (v.getFuel().getType() == Fuel.GAS) ? gasPrice : electricityPrice));

            v.setTotalCost(computeLeaseCost(v.getLease().getDueAtSigning(), v.getLease().getLeaseLength(), v.getLease().getMonthlyCost()) + v.getFuelCost() + ((v.getFuel().getType()==Fuel.ELECTRIC)?v.getFuel().getCharger():0));
        }
    }
